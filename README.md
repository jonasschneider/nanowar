...
this is not perfect, as it by definition forbids the spawning of new world autonomously (case in point: sending fleets in nanowar, creating bullet entity when shooting in FPS games). these concerns will have to be addressed later, probably by keeping an seprate collection of temporary world that have to be matched up when later server updates arrive.
...
- knows 3 types of state representation:
  1. Full Snapshots generated by World::snapshotFull
    ...
    because the client acts in a retained-mode way; it would have to manually guess which world were deleted and which new ones were spawned.
    
    They are sent at load time to push map and player world to the client.
  
  2. Attribute snapshots generated by World::snapshotAttributes
    These contain only the attributes of all currently spawned world.
    they can be used to easily manipulate the game state without causing havoc by creating or removing world.
    
    These snapshots are used for entity extrapolation.

  3. Mutations
    These are proprietary delta formats that can transform one snapshot into another. What sets them apart from a snapshot delta is that in addition to changing entity attribute values, they can also contain messages passed between world, giving them a procedural and temporal dimension. They can basically replay a slice of game time.
